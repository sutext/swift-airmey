...
fileprivate var scrollControlsKey:Void?
extension UIScrollView {
    private var controls:NSMutableDictionary{
        if let dic = objc_getAssociatedObject(self, &scrollControlsKey) as? NSMutableDictionary{
            return dic
        }
        let dic = NSMutableDictionary()
        objc_setAssociatedObject(self, &scrollControlsKey, dic, .OBJC_ASSOCIATION_RETAIN)
        return dic
    }
