...
/**
 The sheetModalPresentationController is the middle layer between the presentingViewController
 ...
 The presented view's layout configuration & presentation is defined using the sheetModalPresentable.

 By conforming to the sheetModalPresentable protocol & overriding values
 the presented view can define its layout configuration & presentation.
 */
open class SheetModalPresentationController: UIPresentationController {
    ...
    public struct Constants {
        ...
        static let dragIndicatorSize = CGSize(width: .screenWidth, height: 24)
    }
    ...
    /**
     Last drag pos
     */
    private var lastMaxPostion: Bool?
    
    /**
     Last in Safe area
     */
    private var lastInSafeArea: Bool?
    
    /**
     last pan state
     */
    private var lastPanGestureState: UIGestureRecognizer.State = .possible
    ...
    /**
     Configuration object for sheetModalPresentationController
     */
    private var presentable: SheetModalPresentable? {
        return presentedViewController as? SheetModalPresentable
    }
    ...
    private var backgroundViewColor: UIColor {
        if let color = presentable?.sheetModalBackgroundColor {
            return color
        }
    ...
    /**
     top safearea placeholder color
     */
    private var topSafeAreaBackgroundColor: UIColor {
        if let color = presentable?.topSafeAreaBackgroundColor {
            return color
        }
        return UIColor.black.withAlphaComponent(0.4)
    }
    ...
    private lazy var backgroundView: DimmedView = {
        let view = DimmedView(dimColor: backgroundViewColor)
        view.didTap = { [weak self] _ in
            if self?.presentable?.allowsTapToDismiss == true {
                self?.presentedViewController.dismiss(animated: true)
            }
        }
        view.backgroundImage = presentable?.sheetModalBackgroundImage
        view.enablePopOverBackground = presentable?.enablePopOverBackground ?? false
        return view
    }()
     ...
     */
    private lazy var panContainerView: SheetContainerView = {
        let frame = containerView?.frame ?? .zero
        return SheetContainerView(presentedView: presentedViewController.view, frame: frame)
    }()
    ...
    private lazy var dragIndicatorView: UIView = {
        ...
        view.bounds = .init(origin: .zero, size: Constants.dragIndicatorSize)
//        view.setAnyCorners(.init(topLeft: 20, topRight: 20))
        
        view.addSubview(indicatorLineView)
        indicatorLineView.translatesAutoresizingMaskIntoConstraints = false
        indicatorLineView.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
        indicatorLineView.centerYAnchor.constraint(equalTo: view.centerYAnchor).isActive = true
        indicatorLineView.widthAnchor.constraint(equalToConstant: 32).isActive = true
        indicatorLineView.heightAnchor.constraint(equalToConstant: 4).isActive = true
    ...
    private lazy var indicatorLineView: UIView = {
        let lineView = UIView()
        lineView.backgroundColor = UIColor.hex(0x4E4E4E)
        lineView.layer.cornerRadius = 4 / 2.0
        return lineView
    }()
    ...
    private var firstAdjustFrame: Bool = true
    ...
    override public func presentationTransitionWillBegin() {
        ...
        firstAdjustFrame = false
        
        if presentable?.enablePopOverBackground == true {
            backgroundView.starPopOverAnitaion()
        }
    }
...
public extension SheetModalPresentationController {

    /**
     Transition the sheetModalPresentationController
    ...
    /**
     Updates the sheetModalPresentationController layout
     based on values in the sheetModalPresentable
...
private extension SheetModalPresentationController {
    ...
    func layoutPresentedView(in containerView: UIView) {
        ...
        /**
         ⚠️ If this class is NOT used in conjunction with the sheetModalPresentationAnimator
         & sheetModalPresentable, the presented view should be added to the container view
...
//        }
    ...
    func adjustPresentedViewFrame() {
        ...
        panContainerView.frame.size = .init(width: frame.width, height: frame.height - anchoredYPosition)
        
        if ![shortFormYPosition, longFormYPosition].contains(panFrame.origin.y) {
            UIView.animate(withDuration: firstAdjustFrame ? 0.0 : 0.25) {
                let yPosition = panFrame.origin.y - panFrame.height
    ...
    func addDragIndicatorView(to view: UIView) {
        ...
        dragIndicatorView.bottomAnchor.constraint(equalTo: view.topAnchor, constant: -Constants.indicatorYOffset + 0.5).isActive = true
    ...
    func configureViewLayout() {
        ...
        guard let layoutPresentable = presentedViewController as? SheetModalPresentable.LayoutType
            else { return }
...
private extension SheetModalPresentationController {
    ...
    private func updateGestureState(_ state: UIPanGestureRecognizer.State) {
        if state != lastPanGestureState {
            presentable?.didChangePanGesture(state: state)
            lastPanGestureState = state
        }
    }
    ...
    @objc func didPanOnPresentedView(_ recognizer: UIPanGestureRecognizer) {
        updateGestureState(recognizer.state)
    ...
    func shouldPrioritize(panGestureRecognizer: UIPanGestureRecognizer) -> Bool {
        return panGestureRecognizer.state == .began &&
            presentable?.shouldPrioritize(sheetModalGestureRecognizer: panGestureRecognizer) == true
    }
    ...
    func snap(toYPosition yPos: CGFloat) {
        SheetModalAnimator.animate({ [weak self] in
    ...
    func adjust(toYPosition yPos: CGFloat) {
        ...
        let defaultTopOffset = presentable?.topOffset ?? 0
        guard presentedView.frame.origin.y > shortFormYPosition - defaultTopOffset else {
            backgroundView.dimState = .max
            return
        }
    ...
    func adjustSafeArea() {
        /// in safearea change background color
        let originY = presentedView.frame.origin.y.rounded()
        let inSafeArea = originY <= CGFloat.headerHeight.rounded()
        if lastInSafeArea != inSafeArea {
            backgroundView.backgroundColor = inSafeArea ? topSafeAreaBackgroundColor : backgroundViewColor
            backgroundView.enablePopOverBackground = !inSafeArea
            
            if let showDragIndicator = presentable?.showDragIndicator {
                if inSafeArea {
                    indicatorLineView.isHidden = true
                } else {
                    indicatorLineView.isHidden = !showDragIndicator
                }
            }
            lastInSafeArea = inSafeArea
        }
        
        let inMaxPos = (originY <= anchoredYPosition.rounded()) || inSafeArea
        if lastMaxPostion != inMaxPos {
            presentable?.willTransition(in: inMaxPos)
            lastMaxPostion = inMaxPos
        }
    }
    
    private func adjustProgress() {
        /// callback gesture progress
        let dragHeight = longFormYPosition - shortFormYPosition
        let progress = max(min(1, 1 - (dragHeight / anchoredYPosition)), 0)
        if dragProgress == progress { return }
        presentable?.didTransition(to: progress)
...
extension SheetModalPresentationController: UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        if let flag = presentable?.gestureRecognizer(gestureRecognizer, shouldRecognizeSimultaneouslyWith: otherGestureRecognizer) {
            return flag
        }
